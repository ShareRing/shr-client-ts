...
import {toBech32, fromBase64, fromUtf8, toBase64, toUtf8} from "@cosmjs/encoding";
...
export class Secp256k1HdWallet implements OfflineDirectSigner {
   ...
   */
  public static async fromMnemonic(mnemonic: string, options: Partial<Secp256k1HdWalletOptions> = {}): Promise<Secp256k1HdWallet> {
    const mnemonicChecked = new EnglishMnemonic(mnemonic);
   ...
   */
  public static async deserializeWithEncryptionKey(serialization: string, encryptionKey: Uint8Array): Promise<Secp256k1HdWallet> {
    const root = JSON.parse(serialization);
    if (!isNonNullObject(root)) throw new Error("Root document is not an object.");
   ...
   */
  public async serializeWithEncryptionKey(encryptionKey: Uint8Array, kdfConfiguration: KdfConfiguration): Promise<string> {
