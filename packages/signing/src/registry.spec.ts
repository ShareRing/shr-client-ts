...
import {MsgSend as IMsgSend} from "@shareledgerjs/types/cosmos/bank/v1beta1/tx";
import {TxBody} from "@shareledgerjs/types/cosmos/tx/v1beta1/tx";
...
describe("registry demo", () => {
  it("works with a default msg", () => {
    ...
    expect(msgSendDecoded.fromAddress).to.equal(msgSend.fromAddress);
    expect(msgSendDecoded.toAddress).to.equal(msgSend.toAddress);
    expect(msgSendDecoded.amount).to.deep.equal(msgSend.amount);
  });
  ...
  it("works with a custom msg", () => {
    ...
    expect(msgDemoDecoded).to.contains({
      creator: "Me",
      title: "Something with stars",
      body: "la la la"
    });
    expect(Uint8Array.from(msgDemoDecoded.attachment)).to.equalBytes(fromHex("AABBAABB66FE"));
  });
});
